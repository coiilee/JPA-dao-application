...
public class VerificationService {
    ...
    //네이버나 구글 등 이메일을 주고 받는 회사에서 회원가입을 진행하기 위해 접속해서 인증번호를 받아가는 회원이 여러명일 수 있기 때문에
    //String,String = email ,code와 같이
    //이메일을 key(키) 명칭으로 설정하고, 이메일에 전송된 인증번호를 Value(값)으로 묶어서
    //verificationCodes 변수 이름에 저장

    //1. 랜덤으로 지정된 숫자 6자리 만들기
    public String randomCode() {
        Random rand = new Random();
        int randomNum = 100000 + rand.nextInt(900000); //6자리 랜덤 숫자 생성
        return String.valueOf(randomNum);//숫자값을 글자 형식으로 변환해서 전달
        //mail 기본 타입 String이기 때문
    }

    //2. 보낼 메일의 제목과 내용이 들어있는 메서드
