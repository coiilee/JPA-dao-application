...
public class SecurityConfig {
    ...
    //자바에서 만든 PasswordEncode.java 파일에서 비밀번호 암호화 기능을 사용하겠다 설정
    ...
    csrf.disable() 비활성화 -> 개발 단계에서 잠시 비활성화 처리해서 추가적인 암호 기능을 개발자가 풀어내기 보다는
                             코드 작성에 집중할 수 있도록 설정해주는 것

   .authorizeHttpRequests
        HTTP 요청에 권한을 설정하는 메서드
        .anyRequest() = 모든 요청(모든 URL 경로로 들어오는 요청)
        .permitAll()  = 모두 허용
        개발 단계에 있어 코딩에 지장이 생기지 않도록 설정하는 것
        .formLogin(form->form.disable());
        ...
        자동으로 보안 설정이 되면서 관리자 권한 아이디 비밀번호를 작성해야지만 사이트를 이용할 수 있또록 초기에 설정되어있음

        spring-boot-starter-security 에 초기 설정되어 있는 아이디 비밀번호 작성을

        form.disable() -> 작성 비활성화 처리 하는것
